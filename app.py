...
def convert():
    ...
    with zipfile.ZipFile(zip_buffer, 'w') as zf:
        for file in files:
            if conversion_type == "video2mp3" and file_ext in {"mp4", "mov", "avi"}:
            ...
            elif conversion_type == "compress_video" and file_ext in {"mp4", "mov", "avi"}:
                output_path = filepath.rsplit('.', 1)[0] + "_compressed.mp4"
                os.system(f'ffmpeg -i "{filepath}" -vcodec libx264 -crf 24 "{output_path}"')
